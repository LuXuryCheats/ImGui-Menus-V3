...
bool ImGui::MyButtonExXD2(const char* label, const ImVec2& size_arg, bool inuse, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (inuse)
	{
		...
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 2), ImVec2(bb.Max.x, bb.Max.y - 0), ImColor(162, 69, 113, 255));
...
bool ImGui::Tab(const char* label, bool selected)
{
	...
	window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(15, 15, 15));
	window->DrawList->AddRect(bb.Min, bb.Max, ImColor(12, 12, 12), 0.f, 15, 2.f);

	if (selected)
	{
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 2, bb.Min.y + 2), ImVec2(bb.Max.x - 2, bb.Max.y - 2), ImColor(23, 23, 23));
		window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 3, bb.Max.y), ImColor(192, 4, 185));
	}

	ImGui::RenderText(ImVec2(bb.Min.x + 10, bb.Min.y + (size.y / 2 - label_size.y / 2 - 2)), label);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 10), ImColor(200, 200, 200), 5);
	window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, ImColor(255, 255, 255, 255), style.GrabRounding);
	window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 10), ImColor(0, 245, 245), 5);
	//window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x + 5, frame_bb.Min.y + 2), ImVec2(frame_bb.Max.x + 45, frame_bb.Max.y - 2), ImColor(38, 38, 38), 3);
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	RenderTextClipped(ImVec2(frame_bb.Max.x + 5, frame_bb.Min.y), ImVec2(frame_bb.Max.x + 45, frame_bb.Min.y + 20), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
	if (label_size.x > 0.0f)
	//	RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	window->DrawList->AddRect(check_bb.Min + ImVec2(pad + 0.10f, pad + 0.10f), check_bb.Max - ImVec2(pad + 0.10f, pad + 0.10f), ImColor(25, 25, 25), 1);
	window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad + 0.10f, pad + 0.8f), check_bb.Max - ImVec2(pad + 0.10f, pad + 0.10f), ImColor(200, 200, 200), 0);
	if (*v)
	{
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad + 0.10f, pad + 0.8f), check_bb.Max - ImVec2(pad + 0.10f, pad + 0.10f), ImColor(255, 36, 36), 0);
	}
